...
export type GitHubRibbonColor = 'red' | 'green' | 'black' | 'orange' | 'grey' | 'white';
export type GitHubRibbonPosition = 'left' | 'right';
export type GitHubRibbonProps = React.DetailedHTMLProps<React.AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement> & {
  href: string;
  color?: GitHubRibbonColor
  position?: GitHubRibbonPosition
}

export const getRibbonImageURL = (position: GitHubRibbonPosition, color: GitHubRibbonColor = 'black'): string => {
  switch(color) {
    case 'green':
...
export const GitHubRibbon = (props: GitHubRibbonProps) => {
  ...
  const src = React.useMemo(() => getRibbonImageURL(position, color), [position, color])
